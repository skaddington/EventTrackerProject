  ...
  index(): Observable<CatchLog[]> {
    return this.http.get<CatchLog[]>(this.url + '?sorted=true').pipe(
      catchError((err: any) => {
        ...
        return throwError(
          () =>
            new Error('CatchLogService.index(): error retrieving logs: ' + err)
        );
      })
    );
  }
  ...
  show(logId: number): Observable<CatchLog> {
    return this.http.get<CatchLog>(this.url + '/' + logId).pipe(
      catchError((err: any) => {
        console.log(err);
        return throwError(
          () => new Error('CatchLogService.index(): error retrieving log: ' + err)
        );
      })
    );
  }
...
console.log(newLog);
    return this.http.post<CatchLog>(this.url, newLog).pipe(
      catchError((err: any) => {
        console.error(err);
        return throwError(
           () => new Error( 'CatchLogService.create(): error creating  log: ' + err )
        );
      })
    );
  }
